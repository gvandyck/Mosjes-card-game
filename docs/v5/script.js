...
function showGraveyardModal() {
    let myId = 'p1';
    if (myRole === 'p2' || (firebaseEnabled && room && room.p2 === playerId)) myId = 'p2';
    const discard = localState.players[myId].discard;
    const graveyardList = document.getElementById('graveyardList');
    if (graveyardList) {
        if (discard.length === 0) {
            graveyardList.innerHTML = '<div class="smallMuted">Graveyard is empty.</div>';
        } else {
            graveyardList.innerHTML = discard.map((c, i) => `<div style='padding:4px 0;border-bottom:1px solid #222;'>${c.name || c}</div>`).join('');
        }
    }
...
function activatePiecie(index){
  ...
  if (firebaseEnabled && myRole) {
      ...
      // In hotseat, we assume the UI interaction comes from the player who owns the piecies being shown.
      // The UI renders 'myPiecies' based on 'myPlayer'.
      // 'myPlayer' is derived from 'localState.currentTurn' in hotseat usually?
      // Let's check how 'myPlayer' is defined in renderAll.
      // It seems 'myPlayer' is a global or derived variable? No, it's usually defined inside renderAll.
      // Wait, activatePiecie is called from onclick.
      // We need to know which player's piecie list we are accessing.
      // In the current simple implementation, 'activatePiecie' assumes it's acting on 'localState.currentTurn'.
      // But for Snelle Piecies out of turn, this assumption is wrong.
      
      // If I am P2 and it is P1's turn, and I click activate on MY piecie.
      // 'localState.currentTurn' is P1.
      // But I want to activate P2's piecie.
      
      // We need to pass the player ID to activatePiecie or infer it.
      // Since we don't have the player ID passed, we have to rely on context.
      // However, the current code: const p = localState.players[turn];
      // This forces it to be the current turn player.
      
      // FIX: We should check if the piecie at 'index' in the CURRENT TURN player's list matches what we expect?
      // No, that's ambiguous.
      
      // Let's assume 'activatePiecie' is called on the 'currentTurn' player by default,
      // UNLESS we are in network mode where we know our identity.
      
      // If we are in hotseat, we can't really click opponent's buttons easily.
      // So let's just keep 'p = localState.players[turn]' for hotseat default,
      // but if we are in network mode, use 'myRole'.
  }
  ...
  // 'p' is already initialized above, do not redeclare here
  // If not network mode, fallback to current turn (standard behavior)
  if (!firebaseEnabled) {
      p = localState.players[localState.currentTurn];
  }
  ...
  if (localState.pendingDiscard > 0) {
      alert(`You must discard ${localState.pendingDiscard} card(s) first!`);
      return;
  }
...
// Ronald ability: global handler
window.ronaldAbility = function() {
    ...
    // Prompt to select which Food Piecie to discard
    let foodOptions = foodIndexes.map(i=>`<option value="${i}">${myPlayer.hand[i].name}</option>`).join('');
    ...
    modal.innerHTML = `<div style='background:#fff;padding:32px 24px;border-radius:12px;max-width:400px;box-shadow:0 2px 16px #0006;text-align:center;'>
        <h2>Ronald: Discard a Food Piecie</h2>
        <select id='ronaldFoodSelect'>${foodOptions}</select>
        <button id='ronaldDiscardBtn' class='btn' style='margin-left:8px;'>Discard</button>
        <button id='ronaldCancelBtn' class='btn' style='margin-left:8px;'>Cancel</button>
    </div>`;
    document.body.appendChild(modal);
    document.getElementById('ronaldCancelBtn').onclick = function() {
        document.body.removeChild(modal);
    };
    document.getElementById('ronaldDiscardBtn').onclick = function() {
        const idx = parseInt(document.getElementById('ronaldFoodSelect').value);
        ...
        // Now show deck search modal
        showRonaldDeckSearch(myPlayer);
    };
};

// Helper: Show deck search modal
function showRonaldDeckSearch(player) {
    if (!player.deck || player.deck.length === 0) {
        alert('Your deck is empty!');
        return;
    }
    let deckOptions = player.deck.map((c,i)=>`<option value="${i}">${c.name}</option>`).join('');
    let modal = document.createElement('div');
    modal.id = 'ronaldDeckSearchModal';
    modal.style.position = 'fixed';
    modal.style.top = '0';
    modal.style.left = '0';
    modal.style.width = '100vw';
    ...
    modal.style.zIndex = '9999';
    modal.innerHTML = `<div style='background:#fff;padding:32px 24px;border-radius:12px;max-width:400px;box-shadow:0 2px 16px #0006;text-align:center;'>
        <h2>Ronald: Select a Card from Deck</h2>
        <select id='ronaldDeckSelect'>${deckOptions}</select>
        <button id='ronaldAddBtn' class='btn' style='margin-left:8px;'>Add to Hand</button>
        <button id='ronaldCancelDeckBtn' class='btn' style='margin-left:8px;'>Cancel</button>
    </div>`;
    document.body.appendChild(modal);
    document.getElementById('ronaldCancelDeckBtn').onclick = function() {
        document.body.removeChild(modal);
    };
    document.getElementById('ronaldAddBtn').onclick = function() {
        const idx = parseInt(document.getElementById('ronaldDeckSelect').value);
        const selected = player.deck.splice(idx,1)[0];
        player.hand.push(selected);
        player._ronaldRevealUsedThisTurn = true;
  ...
  if (localState.pendingSelection && (isMyTurn || localState.pendingSelection.remainingAction === 'none')) {
      ...
      const finishSelection = () => {
          ...
          if (action === 'discard') {
              remaining.forEach(c => myPlayer.discard.push(c));
              log('Remaining cards discarded');
          } else if (action === 'shuffle') {
              remaining.forEach(c => myPlayer.deck.push(c));
              shuffleLocal(myPlayer.deck);
              log('Remaining cards returned to deck and shuffled');
          } else if (action === 'top') {
              remaining.reverse().forEach(c => myPlayer.deck.push(c));
              log('Remaining cards returned to top of deck');
          }
          
          localState.pendingSelection = null;
      ...
      localState.pendingSelection.cards.forEach((card, idx) => {
          const d = document.createElement('div');
          d.className = 'cardView';
          d.style.cursor = 'pointer';
          d.style.border = '2px solid var(--accent)';
          
          // Handle both Piecie (effect) and Mosje (ability) text
          const description = card.effect || card.ability || '';
          
          d.innerHTML = `
            <div style="font-weight:700">${card.name}</div>
            <div class="smallMuted" style="margin-top:6px">${description}</div>
            <div class="smallMuted" style="margin-top:8px">Cost: ${card.cost||0}</div>
          `;
          
          d.onclick = () => {
              if (localState.pendingSelection.count > 0) {
                  myPlayer.hand.push(card);
                  log(`Selected ${card.name} from top of deck`);
